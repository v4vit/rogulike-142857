...
PRISM_DMG = 22                     # 光棱塔每次伤害
PRISM_COOLDOWN = 0.35              # 光棱塔攻击间隔
MISSILE_DMG = 45                   # 导弹塔伤害（削弱溅射）
MISSILE_COOLDOWN = 2.2             # 导弹塔发射间隔
MISSILE_LOCK = 0.6                 # 导弹锁定/追踪时间
MISSILE_EXPLODE_R = 2.5            # 导弹爆炸半径(格)
LIQUID_DMG = 12                    # 液氮塔伤害
LIQUID_COOLDOWN = 0.9              # 液氮塔攻击间隔
LIQUID_SLOW = 0.5                  # 减速倍率
LIQUID_SLOW_TIME = 2.0             # 减速持续(秒)
LIQUID_FREEZE_CHANCE = 0.25        # 概率发射冰冻弹
LIQUID_FREEZE_R = 2.2              # 冰冻范围(格)
LIQUID_FREEZE_TIME = 2.5           # 冰冻持续(秒)
...
TOWER_DEFS = {
    ...
    "gear":      {"name": "齿轮塔",   "size": 0, "cost": 45, "hp": 0,   "color": "#e3b341", "gear": True,
                  "desc": "只能叠放在机枪/导弹/液氮塔上（点其占用格），每个塔最多2个。每装1个使被加装塔冷却 ×0.7（攻速约提升43%）。"},
    ...
    "prism":     {"name": "光棱塔",   "size": 2, "cost": 90, "hp": 160, "color": "#d2a8ff", "range": 6 * CELL,
                  "desc": "射程6格，锁定范围内最左的敌人持续激光攻击，每次22伤害、约0.35秒一发，输出最高的单体塔。"},
    ...
    "liquid":    {"name": "液氮塔",   "size": 2, "cost": 70, "hp": 130, "color": "#8be9fd", "range": 4 * CELL, "gearable": True,
                  "desc": "攻击范围内敌人造成12伤害并使其减速50%（持续2秒）；有25%概率发射冰冻弹，冰冻周围2.2格内敌人2.5秒。可加装齿轮。"},
...
MON_DEFS = {
    ...
    "bgreen": {"name": "强·绿",  "color": "#3fb950", "r": 20, "speed": 0.7, "hp_mult": 12.0, "atk": 12, "weight": 1, "revive": None, "elite": True, "boom": True, "taunt": True,
               "desc": "强化·绿（第5波起出现）：血量变为普通绿的6倍、体型巨大，且自带嘲讽吸引范围内塔优先攻击它。死亡时爆炸，对周围2.5格内塔造成伤害，并分裂出两只普通绿。"},
    "bblue":  {"name": "强·蓝",  "color": "#58a6ff", "r": 8,  "speed": 2.1, "hp_mult": 1.0, "atk": 10, "weight": 1, "revive": None, "elite": True, "aura": True,
               "desc": "强化·蓝（第5波起出现）：自带加速光环，将6格内友军移速提升至2倍，配合成群敌人会迅速突破防线。"},
...
def random_mon_key(wave=1):
    """按权重随机出怪；白/强白/金球走专门机制不在此池；第 5 波前不出强化球"""
...
def normal_summon_keys():
    """强化白召唤/裂隙出怪的普通怪池：排除白、强化白、金球与所有强化球"""
...
def add_effect(etype, x, y, r=0, color="#bb88ff", life=0.6):
    """记录一个特效（范围圆/传送光效），由 draw_dynamic 绘制并随时间淡出"""
    S.effects.append({"type": etype, "x": x, "y": y, "r": r,
...
def _missile_explode(mi):
    ...
    for w in S.wspawns:
        if w["alive"] and dist2(w, mi) <= R * R:
            ...
            if w["hp"] <= 0:
                remove_wspawn(w)
    add_effect("boom", mi["x"], mi["y"], r=R, color="#ff6b6b", life=0.6)
...
def acquire_target(m):
    ...
    bestd = float("inf")
    for t in S.towers:
        if t.get("removed") or t["kind"] != "taunt":
            continue
        if dist2(m, t) <= TAUNT_RANGE ** 2 and dist2(m, t) < bestd:
            bestd = dist2(m, t)
            best = t
    if best:
        return best
...
def monster_update(m, dt):
    ...
    # 黑色球（含强化黑）：闪现到 10×10 格内最近塔（覆盖当前目标）
...
def update(dt):
    ...
    if budget > 0 and S.spawn["acc"] < budget:
        ...
        while S.spawn["timer"] >= S.spawn["interval"] and S.spawn["acc"] < budget:
            ...
            S.monsters.append(spawn_monster(k))
    ...
    for t in S.towers:
        ...
        if target and t["cool"] <= 0:
            ...
            t["cool"] = cool
            S.bullets.append({"x": t["x"], "y": t["y"], "target": target, "life": BULLET_LIFE})
    ...
    for t in S.towers:
        ...
        if k == "boom_unit":
            ...
            if touched:
                ...
                remove_tower(t)
            continue
        # 光棱塔：锁定射程内怪物持续攻击（优先嘲讽的强化绿）
        if k == "prism":
            t["cool"] -= dt
            target = tower_acquire(t, t["range"])
            if target and t["cool"] <= 0:
                t["cool"] = PRISM_COOLDOWN
                damage_monster(target, PRISM_DMG)
                S.effects.append({"type": "beam", "x": t["x"], "y": t["y"],
                                  "x2": target["x"], "y2": target["y"],
                                  "r": 0, "color": "#d2a8ff",
                                  "life": 0.2, "maxlife": 0.2, "t": 0.0})
            continue
        ...
        if k == "liquid":
            ...
            if t["cool"] <= 0:
                target = tower_acquire(t, t["range"])
                if target:
                    cool = LIQUID_COOLDOWN * (GEAR_COOLDOWN_FACTOR ** t.get("gears", 0))
                    t["cool"] = cool
                    damage_monster(target, LIQUID_DMG)
                    target["slow"] = max(target["slow"], LIQUID_SLOW_TIME)
                    # 概率发射范围冰冻弹
                    if random.random() < LIQUID_FREEZE_CHANCE:
                        FR = LIQUID_FREEZE_R * CELL
                        for m in S.monsters:
                            if m["alive"] and dist2(m, target) <= FR * FR:
                                m["frozen"] = LIQUID_FREEZE_TIME
                        add_effect("boom", target["x"], target["y"], r=FR,
                                   color="#8be9fd", life=0.6)
                        log("❄ 液氮塔发射冰冻弹!", "info")
            continue
        # 治疗塔：治疗范围内塔 + 套盾
        if k == "healer":
    ...
    for m in S.monsters:
        ...
        for other in S.monsters:
            ...
            if dist2(other, m) <= R * R:
                other["boosted"] = True
                other["boost_speed"] = other["speed"] * BBLUE_BOOST   # 2 倍速
    ...
    for w in S.wspawns:
        ...
        if w["timer"] <= 0:
            w["timer"] = w["interval"]
            nm = spawn_monster(random_mon_key(S.wave), x=w["x"], y=w["y"])
            S.monsters.append(nm)
...
class Game:
    ...
    def draw_tower(self, t):
        ...
        icon = {"solar": "☀", "gun": "🔫", "taunt": "❗",
                ...
                "liquid": "🧊", "healer": "⛑", "energy": "⚡"}.get(t["kind"], "?")
        self.canvas.create_text(x, y, text=icon, fill="#111",
                                font=("Segoe UI Emoji", int(size * CELL * 0.5)),
                                tags="tower")
        # 可加装齿轮的塔（机枪/导弹/液氮）叠加齿轮标记
        if t["kind"] in ("gun", "missile", "liquid"):
            ng = t.get("gears", 0)
            for k in range(ng):
                gx = x + (k - (ng - 1) / 2.0) * 16
                gy = y + size * CELL * 0.28
    ...
    def draw_dynamic(self):
        ...
        for m in S.monsters:
            ...
            self.canvas.create_oval(m["x"] - m["r"], yy - m["r"],
                                    ...
                                    tags="dyn")
            # 眼睛：普通球白瞳，强化球红瞳（红眼特效）
            eye = "#f85149" if elite else "#d0d0d0"
    ...
    def on_click(self, e):
        ...
        if 0 <= c < GRID and 0 <= r < GRID:
            if place_tower(S.sel, c, r):
                if S.sel == "gear":
                    self.redraw_static()   # 齿轮叠在机枪塔上，需重绘塔层显示齿轮
                else:
...
class App:
    ...
    def _codex_row(self, parent, color, name, info, desc=""):
